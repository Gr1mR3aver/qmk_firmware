...
bool qp_st77xx_circle_drawpixels(painter_device_t device, uint16_t centerx, uint16_t centery, uint16_t offsetx, uint16_t offsety, uint8_t hue, uint8_t sat, uint8_t val, bool filled) {
    ...
    */
    ...
    bool retval = true;
    if (offsetx == 0) {
        if (!qp_st77xx_internal_setpixel_impl(lcd,centerx, centery + offsety, hue, sat, val)) {
            retval = false;
        }
        if (!qp_st77xx_internal_setpixel_impl(lcd,centerx, centery - offsety, hue, sat, val)) {
            retval = false;
        ...
        if (filled) {
            if (!qp_st77xx_rect(lcd, centerx + offsety, centery, centerx - offsety, centery, hue, sat, val, true)) {
                retval = false;
            }
        } else {
            if (!qp_st77xx_internal_setpixel_impl(lcd,centerx + offsety, centery, hue, sat, val)) {
                retval = false;
            }
            if (!qp_st77xx_internal_setpixel_impl(lcd,centerx - offsety, centery, hue, sat, val)) {
                retval = false;
        ...
        if (filled) {
            ...
            if (!qp_st77xx_internal_filled_rect_impl(lcd, centerx + offsety, centery + offsety, centerx - offsety, centery + offsety, hue, sat, val, buf, bufsize)) {
                retval = false;
            }
            if (!qp_st77xx_internal_filled_rect_impl(lcd, centerx + offsety, centery - offsety, centerx - offsety, centery - offsety, hue, sat, val, buf, bufsize)) {
                retval = false;
            ...
            if (!qp_st77xx_internal_setpixel_impl(lcd,centerx + offsety, centery + offsety, hue, sat, val)) {
                retval = false;
            }
            if (!qp_st77xx_internal_setpixel_impl(lcd,centerx - offsety, centery + offsety, hue, sat, val)) {
                retval = false;
        ...
        if (filled) {
           ...
           if (!qp_st77xx_internal_filled_rect_impl(lcd, centerx + offsetx, centery + offsety, centerx - offsetx, centery + offsety, hue, sat, val, buf, bufsize)) {
                retval = false;
            }
            if (!qp_st77xx_internal_filled_rect_impl(lcd, centerx + offsetx, centery - offsety, centerx - offsetx, centery - offsety, hue, sat, val, buf, bufsize)) {
                retval = false;
            }
            if (!qp_st77xx_internal_filled_rect_impl(lcd, centerx + offsety, centery + offsetx, centerx - offsety, centery + offsetx, hue, sat, val, buf, bufsize)) {
                retval = false;
            }
            if (!qp_st77xx_internal_filled_rect_impl(lcd, centerx + offsety, centery - offsetx, centerx - offsety, centery - offsetx, hue, sat, val, buf, bufsize)) {
                retval = false;
            ...
            if (!qp_st77xx_internal_setpixel_impl(lcd,centerx + offsetx, centery + offsety, hue, sat, val)) {
                retval = false;
            }
            if (!qp_st77xx_internal_setpixel_impl(lcd,centerx - offsetx, centery + offsety, hue, sat, val)) {
                retval = false;
            ...
            if (!qp_st77xx_internal_setpixel_impl(lcd,centerx + offsety, centery + offsetx, hue, sat, val)) {
                retval = false;
            }
            if (!qp_st77xx_internal_setpixel_impl(lcd,centerx - offsety, centery + offsetx, hue, sat, val)) {
                retval = false;
